...
namespace osu.Game.Rulesets.Kumori.BPM
{
    public partial class SettingsBPMAudioMode : SettingsEnumDropdown<BPMAdjustAudioMode>
    {
        ...
        private void load()
        {
            var mod = (KumoriModBPMAdjust)SettingSourceObject;
        ...
        private static string getTreatmentHint(BPMAdjustAudioMode mode) => mode switch
        {
            BPMAdjustAudioMode.PreservePitch => "Natural pitch; large BPM changes require heavier time stretching.",
            BPMAdjustAudioMode.AdjustPitch => "Cleanest processing; pitch changes directly with playback speed.",
            BPMAdjustAudioMode.Balanced => "Splits the BPM change evenly between pitch and time stretching.",
            BPMAdjustAudioMode.Adaptive => "Preserves pitch from 0.75x to 1.5x, then adds only the pitch needed for cleaner extremes.",
            BPMAdjustAudioMode.CustomPitch => "Uses the selected pitch and compensates tempo to keep the target BPM exact.",
            _ => string.Empty,
        };
    }
}
